...
test('catalog evidence is aggregated without exposing source names',()=>{const r=query('цепь для бензопилы 72 звена');assert.equal(r.candidates[0].code,'8202400000');assert(r.candidates[0].evidence.items>0);assert.equal(JSON.stringify(r.candidates[0].evidence).includes('.xlsx'),false);});
